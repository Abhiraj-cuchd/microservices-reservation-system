...
export abstract class AbstractRepository <TDocument extends AbstractDocument> {
  ...
  async findOneAndUpdate(
    ...
    this.logger.log(`Finding one and updating document with filter: ${JSON.stringify(filterQuery)} and update: ${JSON.stringify(update)}`);
    const document = await this.model.findOneAndUpdate(filterQuery, update, {
      new: true,
    }).lean<TDocument>();
    if (!document) {
      this.logger.warn(`Document not found for update with filter: ${JSON.stringify(filterQuery)}`);
      throw new NotFoundException('Document was not Found');
    }
    return document;
  }
  ...
  async findOneAndDelete(filterQuery: FilterQuery<TDocument>): Promise<TDocument> {
    this.logger.log(`Deleting document with filter: ${JSON.stringify(filterQuery)}`);
